...
class Indices
{
    ...
    constructor() {
        ...
        this.userIdFieldName = config.getItem(".framework.users.user_id_field_name").valueStr();
    }
     ...
     */
    #createListDefinition(recordItem, userRole) {
        ...
        let nameFields = (new Record(recordItem)).templateFields("name");
        let selectString = "";
        if (recordItem.hasChild("uid"))
            selectString += ",uid";
        if (recordItem.hasChild("uuid"))
            selectString += ",uuid";
        if (recordItem.hasChild("invalid_from"))
            selectString += ",invalid_from";
        if (recordItem.hasChild(this.#userIdFieldName))
        ...
        let optionsString = (recordItem.hasChild("invalid_from"))
            ? "sort=uuid.-invalid_from&firstofblock=uuid" : "sort=" + sortField;
        for (let nameField in nameFields)
    ...
    #addTable(recordItem, list) {
        ...
        let listDefinition = this.#createListDefinition(recordItem, userRole);
