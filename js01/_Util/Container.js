 ...
 * margin are always 0px. If a container holds no children boxes, it becomes a
...
class Container
{
    ...
    static buildLayout(uiSettingsCfg) {
    	if (uiSettingsCfg) 
        	Container.#uiSettingsCfg = uiSettingsCfg;
    	if (!Container.#uiSettingsCfg)
    		return;
    	uiSettingsCfg = Container.#uiSettingsCfg;
    	if (!uiSettingsCfg.getChild("preferences").hasChild("profile")) {
    		alert("Missing UI profile entry in user preferences");
    		return;
    	}
    	this.#profileName = uiSettingsCfg.getChild("preferences").getChild("profile").value();
    	if (!uiSettingsCfg.getChild("profiles").hasChild(this.#profileName)) {
    		alert(i18n.t("Missing UI profile '%1'", this.#profileName));
    		return;
    	}
    	let profileCfg = uiSettingsCfg.getChild("profiles").getChild(this.#profileName);
    	Container.#layoutType = Container.#getLayoutType();
    	if (!profileCfg.hasChild(Container.#layoutType)) {
    		alert(i18n.t("Missing layout type '%1' in UI profile", Container.#layoutType));
    		return;
    	}
    	let layoutName = profileCfg.getChild(Container.#layoutType).getValue();
    	if (!uiSettingsCfg.getChild("layouts").hasChild(layoutName)) {
    		alert(i18n.t("Missing layout '%1' in UI profile", layoutName));
    		return;
    	}
     	// initialise the container index. It will be necessary for resizing.
    	Container.#containerIndex = {};
    	Container.#layoutRootCfg = uiSettingsCfg.getChild("layouts").getChild(layoutName);
    	// create the root container
    	Container.#layoutRootContainer = new Container(Container.#layoutRootCfg, null);
    	// Clear the root container contents to rebuild it.
		let jQuerySelector = '#' + Container.#layoutRootContainer.#ownId
    	$(jQuerySelector).html("");
    	// set the class of the root container to ensure absolute positioning.
    	$(jQuerySelector).addClass("tfyh-container");
    	// set the global margin as padding in pixels
    	...
    	Container.#layoutRootContainer.#setDimensions(window.innerWidth - 2 * mpP,
    			window.innerHeight - 2 * mpP, mpP, mpP, mpP);
    	// create the layout itself via children drilldown
    	Container.#layoutRootContainer.#createOrResizeChildren(true);
    	// add all contents providers
    	for (let containerId in Container.#containerIndex) {
    		let container = Container.#containerIndex[containerId];
    		container.#provider = (container.#providerCfg) ? new DilboUIprovider(container, container.#providerCfg) : null;
    	}
    	Container.#buildCompleted = true;
    	return Container.#layoutRootContainer;
    }
    ...
    static onResize() {
    	...
    	if (!Container.#buildCompleted)
    		return;
    	let newLayoutType = Container.#getLayoutType();
    	if (newLayoutType !== this.#layoutType) {
			 ...
			 * portrait to landscape enlarges it.
			 */ 
    	...
    	Container.#layoutRootContainer.#setDimensions(window.innerWidth - 2 * mpP,
    			window.innerHeight - 2 * mpP, mpP, mpP, mpP);
    	Container.#layoutRootContainer.#createOrResizeChildren(false);
    }
    ...
    constructor (containerCfg, parentId)
    {
    	...
    	this.#horizontal = this.#cfg.getChild("horizontal").value();
    	...
    	// identify the html ids for the own and parent container
   		...
   		Container.#containerIndex[this.#ownId] = this;
   		this.#parentId = parentId;
   		// check the provider
   		let providerName = this.#cfg.getChild("provider").value();
    ...
    #createDiv() {
    	let div = $("#" + this.#ownId);
    	if (div.length === 0) {
			let jQuerySelector = "#" + this.#parentId
			let parentHtmlSoFar = $(jQuerySelector).html();
			let htmlClass = (this.#isTile) ? "tfyh-tile" : "tfyh-container";
			let innerHtml = (this.#isTile) ? this.#cfg.get_uid() : "";
			$(jQuerySelector).html(parentHtmlSoFar 
    ...
    #createOrResizeChildren(create) {
    	// set the top left corner of the first child. Always relative
    	let childTopP = 0;
    	let childLeftP = 0;
    	if (!create) {
    	...
    	for (let weightObject of this.#childrenWeights) {
    		...
    		let childCfg = this.#cfg.getChild(weightObject.name);
    		let childContainer;
    		if (create) {
    			childContainer = new Container(childCfg, this.#ownId);
    			childContainer.#createDiv();
    		} else 
    			childContainer = Container.#containerIndex[weightObject.id];
    ...
    #getChildrenWeightObjects() {
    	...
    	for (let cname in this.#cfg.get_children()) {
    		let child = this.#cfg.getChild(cname);
    		if (child.get_type().startsWith(".")) 
    			this.#childrenWeights.push( { 
    				name : cname, 
    				id : "tfyh-" + child.get_uid(), 
    				weight : child.getChild("weight").value(),
    				sizeMinP : child.getChild("size_min").value(),
	 ...
	 * that weightObject sets the size in pixel to 0 and returns true. If no such
    ...
    /**
	 * Set the own dimensions and its children sizes.
	 */
